...
class TargetConditionedRouter(nn.Module):
    ...
    def __init__(
        ...
        # When target_emb_dim == 0 the router is target-agnostic; skip the
        # embedding entirely (a zero-width nn.Embedding triggers a CUDA
        # illegal-memory-access on some GPU/torch builds).
        if target_emb_dim > 0:
        ...
        router_in = summary_dim + target_emb_dim
        ...
        self.noise_gates = nn.Linear(router_in, n_experts)
...
class MoEPredictor(nn.Module):
    ...
    def forward(
        ...
        y = (mix + bias) if self.use_target_bias else mix
...
def make_default_moe(
    ...
    return MoEPredictor(
        ...
        dropout=dropout,
        use_target_bias=use_target_bias,
    )
