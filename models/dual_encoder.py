...
class DualEncoder(nn.Module):
    ...
    def __init__(self, node_features=75, edge_features=6, vocab_size=50,
        ...
        # Token projection used ONLY by the no-cross-attention ablation path so
        # that the sequence stream uses the same _pool_tokens pooling as the
        # attention path (the cross-attention module does this projection
        # internally). Unused by 'attention'; absent from older checkpoints, so
        # load with strict=False.
        self.seq_proj_ablate = nn.Sequential(
            nn.Linear(seq_token_dim, output_dim), nn.LayerNorm(output_dim), nn.ReLU(),
        )
    ...
    def forward(self, graph_data, smiles_data, smiles_lengths=None,
        ...
        """
        # --- Pre-pooling embeddings (the graph encoder's own pooled vector is
        #     discarded; we always pool through the stable _pool_nodes head). ---
        ...
        # --- Compute the pooled graph (g) and sequence (s) vectors. The ONLY
        #     difference across fusion types is whether cross-attention is
        #     applied before pooling; the pooling heads and final fusion_proj
        #     are identical, so the ablation isolates the cross-attention. ---
        ft = getattr(self, 'fusion_type', 'attention')
        attn_g2s = None
        if ft == 'attention':
            H_graph_att, H_seq_att, attn_g2s = self.cross_attention(
                node_emb, batch_vec, token_emb, smiles_lengths
            )
            g = self._pool_nodes(H_graph_att, batch_vec)
            s = self._pool_tokens(H_seq_att, smiles_lengths)
        else:  # no cross-attention: same pooling on raw embeddings
            g = self._pool_nodes(node_emb, batch_vec)
            s = self._pool_tokens(self.seq_proj_ablate(token_emb), smiles_lengths)

        if ft == 'graph_only':
            fused = self.fusion_proj(g)
        elif ft == 'seq_only':
            fused = self.fusion_proj(s)
        else:  # 'attention' or 'no_xattn': identical gated fusion
            lam = torch.sigmoid(self.gate_proj(torch.cat([g, s], dim=-1)))
            fused = self.fusion_proj(lam * g + (1 - lam) * s)
