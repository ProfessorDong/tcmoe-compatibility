...
TARGET_PATHS = {
    ...
    # Expanded targets for the source--target compatibility study (v8); curated
    # by curate_chembl_v8.py. Not part of ALL_TARGETS (the core 4-target set).
    'drd3':  os.path.join(BASE, 'data', 'drd3_chembl.csv'),
    'htr2a': os.path.join(BASE, 'data', 'htr2a_chembl.csv'),
    'oprm1': os.path.join(BASE, 'data', 'oprm1_chembl.csv'),
    'oprk1': os.path.join(BASE, 'data', 'oprk1_chembl.csv'),
    'cnr2':  os.path.join(BASE, 'data', 'cnr2_chembl.csv'),
    'adora2a': os.path.join(BASE, 'data', 'adora2a_chembl.csv'),
    'faah':  os.path.join(BASE, 'data', 'faah_chembl.csv'),
    'ptgs2': os.path.join(BASE, 'data', 'ptgs2_chembl.csv'),
    'ache':  os.path.join(BASE, 'data', 'ache_chembl.csv'),
    'egfr':  os.path.join(BASE, 'data', 'egfr_chembl.csv'),
    'kdr':   os.path.join(BASE, 'data', 'kdr_chembl.csv'),
    'herg':  os.path.join(BASE, 'data', 'herg_chembl.csv'),
}
...
def train_moe(
    ...
    patience=20, load_balance=0.01, dropout=0.2, use_target_bias=True,
):
    moe = make_default_moe(
        n_targets=n_targets, top_k=top_k, target_emb_dim=target_emb_dim,
        load_balance=load_balance, dropout=dropout, use_target_bias=use_target_bias,
    ).to(device)
...
def run_topk_ablation(args, device, encoder_emb_cache, target_splits=None):
    ...
    configs = [
        ...
        {'name': 'topk=3, target-cond',  'top_k': 3, 'target_emb': args.target_emb_dim},
